...
import {
    ...
    IconButton, Tooltip, Grow, Avatar, makeStyles, Backdrop, CircularProgress
            ...
            <Dialog TransitionComponent={Grow} open={settingsdialog} onClose={() => setsettingsdialog(false)}>
                <DialogContent>
                    ...
                    <div className="settingsdialog__avatardiv">
                        ...
                        <input type="file" ref={photoChangeRef} accept="image/*" style={{ display: "none" }} onChange={(e) => {
                            if (e.target.files[0]) {
                                if (!e.target.files[0].type.includes("image")) return dispatch(setsnackbar({
                                    snackbar: {
                                        open: true,
                                        type: "error",
                                        filesizeerror: true,
                                ...
                                if (e.target.files[0]?.size < 52428800) {
                                    ...
                                    dispatch(setsnackbar({
                                        snackbar: {
                                            ...
                                            type: "error",
                                            filesizeerror: true,
                                            hu: `A fotó mérete ${formatBytes(e.target.files[0].size)}, amely meghaladja a maximális méretet! (50 MB)`,
                                            en: `Photo size is ${formatBytes(e.target.files[0].size)}, which exceeds the maximum size! (50 MB)`,
                                        }
                                    }))
                            }
