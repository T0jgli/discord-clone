...
const SidebarCategories = ({ categorie, categorieid }) => {
  ...
  const [hide, sethide] = useState(
    hiddencategories
      ? hiddencategories.includes(categorieid)
        ? true
        : false
      : false
  );
    ...
    if (!hide)
      if (hiddencategories)
        localStorage.setItem(
          "hiddenCategories",
          JSON.stringify([...hiddencategories, categorieid])
        );
      else
        localStorage.setItem("hiddenCategories", JSON.stringify([categorieid]));
    else
      localStorage.setItem(
        "hiddenCategories",
        JSON.stringify(hiddencategories.filter((e) => e !== categorieid))
      );
  };
      ...
      <div className="sidebar__channelsheader">
        <div className="sidebar__header" onClick={hideCategorie}>
          <ExpandMoreIcon
            className={
              hide
                ? "sidebar__categorieiconshowed sidebar__menuicon"
                : "sidebar__menucion"
            }
          />
